...
echo -e "You're deploying observatory to ${RED}production!${NC}"
...
docker buildx build --platform=linux/amd64 -t "ghcr.io/beingflo/observatory:${newVersion}" .
if [[ $? -ne 0 ]]; then
...
docker push "ghcr.io/beingflo/observatory:${newVersion}"
if [[ $? -ne 0 ]]; then
...
grep -q "image: \"ghcr.io/beingflo/observatory:${cleanVersion}\"" ./docker-compose.prod.yml
if [[ $? -ne 0 ]]; then
...
sed -i '' -e "s/image: \"ghcr.io\/beingflo\/observatory:${cleanVersion}\"/image: \"ghcr.io\/beingflo\/observatory:${newVersion}\"/" ./docker-compose.prod.yml

## Upgrade image running on omni
echo "Upgrading observatory on omni"
